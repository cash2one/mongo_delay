import gzip,requests,json,time,setting,base64
...
class request_server:
        ...
        if data.count():
            for task in data:
                try:
                    ...
                    self.db[setting.TASKS_LIST].update({setting.ROW_GUID: task['guid'],setting.ROW_STATUS:{'$in':[4,5]}},
    ...
    def send_data(self):#发送数据接口，返回服务器的回复，通信正确返回的是一个字典
        try:
            ...
            text = requests.post("http://118.187.53.58:5500/", json=self.head)
            return text
        except:
            return '服务器数据异常'
