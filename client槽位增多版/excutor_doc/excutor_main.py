...
class excutor_cls:
    ...
    def interface(self, dict,timeout):  # 参数必须是一个字典
    ...
    def get_proxy(self):  # 获取代理
        ...
        proxyip = self.db_obj.find_modify(proxy_tb,{'ipInfo.status': 1, 'usetime': {'$lte': (time.time() - 5 * 60)}},
                                          {'$set': {'usetime': time.time()}, '$inc': {'ipInfo.count': 1}})
        if proxyip:  # 得到代理
            return proxyip
        else:
            self.db_obj.much_update_data(proxy_tb,{'ipInfo.status': 4},{'$set': {'ipInfo.count': 0, 'ipInfo.status': 1}})
             # 将状态4的代理更新
