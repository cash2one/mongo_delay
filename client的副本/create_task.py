...
import time,uuid
from excutor_doc.db_oprate import *
class jd_task_kind:
    ...
    @classmethod
    def generateTask1(cls):  # getVerificateTask(cls):
        # 检查数据库,将大于验证时间的代理挑出来
        # proxylist = []
        obj = collection_db()
        ptb = obj.choice_table('jame_proxy1')
        arr = ptb.find({})
        for index, item in enumerate(arr):
            item.pop('_id')

            ptlstr = item.get('ipInfo').get('ptl').lower()
            ptllist = ptlstr.split('/')
            if len(ptllist) == 1:
                ptllist = ptllist[0].split(',')
            for i in range(len(ptllist)):
                item['ipInfo']['ptl'] = ptllist[i]
                task_proxyip = {'device': {'type': '', 'version': '127.22', 'id': ''},
                                ...
                                'suspend': 0,  # 暂停标识
                                'status': 0,
                                'body': item  # myip.get("ipInfo")
                                }
                #   print (task_proxyip)
                ttb = obj.choice_table('task_main')
                obj.insert_data(ttb, task_proxyip)
