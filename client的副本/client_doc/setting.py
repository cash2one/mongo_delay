...
CRAWL_EXCUTOR_MODE = PYTHON_CRAWL_EXCUTOR#指定选用的抓取器

#相对于抓取器客户端相对于服务器
CRAWL_EXCUTOR_IP = '192.168.0.99'#使用go抓取器的时候并且抓取器放在远端需要修改该IP为本地IP,（后续补充）程序入口会判断这个ip，127.0.0.1ip启动本地go抓取器，如果是本季ip不启动本地go抓取器
