    ...
    def pop_task(self):  # db数据库，topic任务类型
        ...
        for topic in setting.TOPIC:#轮询任务类型
            ...
            if data:  # 得到就绪任务
                ...
                result = self.db_obj.find_modify(task_tb,{setting.ROW_GUID:data[setting.ROW_GUID]},{'$set': {setting.ROW_STATUS: setting.STATUS_EXCUTING}})
                 # 进入总任务列表修改任务状态为正在执行
                break#得到一个任务就返回
        return result
    ...
    def scan_localtask(self):#负责扫描本地任务的进程
        ...
        while True:
            try:
                ...
                if data:
                    task_tb = self.db_obj.choice_task_table()
                    result = self.db_obj.find_one(task_tb,{setting.ROW_GUID: data[setting.ROW_GUID], setting.ROW_TOPIC: data[setting.ROW_TOPIC]})
                     #获取本地任务，调用本地任务脚本
                    module_name = '.'.join((setting.SCRIPT_DIR, topic))
                    try:
                        m1 = __import__(module_name)  # 找到了脚本所在的目录
                    ...
                    self.db_obj.find_modify(task_tb,{setting.ROW_GUID: data[setting.ROW_GUID],setting.ROW_TOPIC: data[setting.ROW_TOPIC],},{
                                                                        '$set': {
                                                                            'status': 0,
